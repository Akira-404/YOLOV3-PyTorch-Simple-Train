...
image_ids = open(
    os.path.join(_conf['dataset_root'], "VOCdevkit/VOC2007/ImageSets/Main/test.txt")).read().strip().split()
...
for image_id in tqdm(image_ids):
    with open(os.path.join(_map_out_path, "ground-truth/" + image_id + ".txt"), "w") as new_f:
        root = ET.parse(
            os.path.join(_conf['dataset_root'], "VOCdevkit/VOC2007/Annotations/" + image_id + ".xml")).getroot()
