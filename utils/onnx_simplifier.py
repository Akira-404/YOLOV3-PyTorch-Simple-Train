...
"""
@Project ：onnx
@File    ：onnx_simplifier.py
@Author  ：Linfeng Lee
@Date    ：2021/7/15 下午2:20 
"""
...
def get_constant_nodes(m: onnx.ModelProto, dynamic_input_shape: bool = False) -> List[onnx.NodeProto]:
    ...
    def is_dynamic(node):
        if node.op_type in ['NonMaxSuppression', 'NonZero', 'Unique'] and node.input[0] not in const_tensors:
            return True
        if node.op_type in ['Reshape', 'Expand', 'Upsample', 'ConstantOfShape'] and len(node.input) > 1 and node.input[
            1] not in const_tensors:
            return True
        if node.op_type in ['Resize'] and ((len(node.input) > 2 and node.input[2] not in const_tensors) or (
                len(node.input) > 3 and node.input[3] not in const_tensors)):
            return True
        return False
...
def check_and_update_input_shapes(model: onnx.ModelProto, input_shapes: TensorShapesWithOptionalKey,
                                  dynamic_input_shape: bool) -> TensorShapes:
    input_names = get_input_names(model)
